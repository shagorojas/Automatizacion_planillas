...
class GeneradorPlantillas:
    ...
    def crear_plantilla_control(self, df_plantilla, nombre_df, var_sede, var_codigo_dane_sede, var_jornada, df_encabezado, var_grado, df_conteo_sede, df_parametros, df_focalizacion):
        ...
        if params["municipio_proceso"] == "FACA":
            worksheet.write('E12', 'GRADO:', workbook.add_format({
                'bold': True,
                'align': 'center',
                'valign': 'vcenter',
                'font_size': 12
            ...
            worksheet.write('F12', var_grado, borde_inferior)
        ...
        # Definir la fila y la columna inicial
        fila_inicio = 17  # Iniciar en la fila 17 en Excel
        col_inicio = 10   # Iniciar en la columna 10 (K en Excel)

        # Iterar sobre las columnas en columnas_despues y escribir sus valores en la hoja de Excel
        for j, columna in enumerate(columnas_despues):  
            if columna in df_plantilla.columns:  # Verificar que la columna existe en df_plantilla
                for idx, doc_value in enumerate(df_plantilla[columna]):  
                    row = fila_inicio + idx  # Iniciar desde la fila 17 y aumentar en cada iteración
                    col_actual = col_inicio + j  # La columna inicial es 10 y aumenta dinámicamente
                    # Reemplazar NaN o valores no soportados con una cadena vacía
                    if pd.isna(doc_value) or isinstance(doc_value, (float, int)) and not np.isfinite(doc_value):
                        doc_value = ""

                    worksheet.write(row - 1, col_actual, doc_value, formato_arial_borde)
        ...
        if params["municipio_proceso"] == "FUNZA":
            ...
            # Insertar valores de la columna 'GRUPO' del DataFrame en la columna I, de la fila 17 a la 36
            for idx, doc_value in enumerate(df_plantilla['GRUPO']):  
                row = 17 + idx  # Iniciar en la fila 17
                worksheet.write(row - 1, 8, doc_value, formato_arial_borde)  # Columna 'I' corresponde al índice 8
        ...
        worksheet.merge_range('G46:P46', 'FIRMA RECTOR ESTABLECIMIENTO EDUCATIVO', 
                            workbook.add_format({
                                'bold': True,
                                'align': 'center',
                                'valign': 'vcenter',
        ...
        # ===================================================================
        # Logica sumatoria total de días de consumo
        # ===================================================================

        # Formato con borde inferior
        borde_inferior = workbook.add_format({'bottom': 1})
        
        # Crear un formato con Arial 14 y bordes
        formato_arial = workbook.add_format({
            'font_name': 'Arial',
            'font_size': 14,
            'align': 'center',  # Alinear al centro
            'valign': 'vcenter', # Alinear verticalmente al centro
        ...
        cantidad = df_conteo_sede.loc[
            (df_conteo_sede["SEDE"] == var_sede) & 
            (df_conteo_sede["JORNADA"] == var_jornada) &
            (df_conteo_sede["GRADO_COD"] == var_grado), 
            "CANTIDAD"
        ].values

        # Verifica si hay valores antes de intentar extraer
        cantidad_filas = int(cantidad[0]) if len(cantidad) > 0 else 0

        # ==================================================
        # CUANDO SOLO SE REQUIERE CONTAR X DE LA PLANILLA
        # ==================================================
        # Contar la cantidad de filas de df_plantilla 
        # cantidad_filas = len(df_plantilla)

        # ===================================================
        # ===================================================

        # Contar la cantidad de elementos en columnas_despues
        cantidad_columnas = len(columnas_despues)

        # Calcular el resultado
        resultado = cantidad_filas * cantidad_columnas

        # Obtener el valor del primer registro de la columna 'TIPO DE RACIÓN'
        valor_J17 = df_plantilla['TIPO DE RACIÓN'].iloc[0] if 'TIPO DE RACIÓN' in df_plantilla.columns and not df_plantilla.empty else None

        # Escribir el resultado en la celda correspondiente según el valor de TIPO DE RACIÓN
        if resultado > 0 and valor_J17:
            # Verifica si "RPS" está contenido en valor_J17
            if "RPS" in valor_J17: 
                # Escribir el resultado en la celda D38
                worksheet.write(37, 3, resultado, formato_arial)  # D38

                # Lista de celdas a modificar
                celdas = ['D40', 'D42']

                # Aplicar solo el formato sin borrar el contenido
                for celda in celdas:
                    worksheet.write_blank(celda, None, borde_inferior)

            elif "CCT" in valor_J17:
                # Escribir el resultado en la celda D40
                worksheet.write(39, 3, resultado, formato_arial)  # D40
            ...
            elif "RI" in valor_J17:
                # Escribir el resultado en la celda D42
                worksheet.write(41, 3, resultado, formato_arial)  # D42
        ...
        # ===================================================================
        # Logica sumatoria X marcadas acumuladas
        # ===================================================================

        # Definir la columna donde se colocará la suma de "X" en Excel (AH = columna 33 en Excel)
        columna_suma_X = 33  

        # Inicializar el contador para la sumatoria total de "X"
        total_X = 0  

        # Crear un diccionario para almacenar el conteo de "X" por columna
        conteo_por_columna = {col: 0 for col in columnas_despues if col in df_plantilla.columns}

        # Recorrer las filas desde 17 hasta 36 para contar las "X"
        for i in range(17, 37):  
            idx = i - fila_inicio  # Ajuste del índice para el DataFrame

            if idx < len(df_plantilla):  # Asegurar que no se salga del índice del DataFrame
                # Contar cuántas veces aparece "X" en cada columna y actualizar el diccionario
                conteo_X = 0
                for col in columnas_despues:
                    if col in df_plantilla.columns and df_plantilla[col].iloc[idx] == "X":
                        conteo_por_columna[col] += 1
                        conteo_X += 1  # Acumulado por fila

                # Acumular el conteo total de "X"
                total_X += conteo_X

                # Si la suma es 0, dejar la celda vacía
                valor_suma = conteo_X if conteo_X > 0 else ""

                # Escribir la suma en la celda AH:AI sin volver a fusionar
                worksheet.write(i - 1, columna_suma_X, valor_suma, formato_arial_borde)

        # ===================================================================
        # Logica para Novedad cambio tipo complemento
        # ===================================================================

        # Filtrar solo esas columnas
        df_filtrado = df_focalizacion[columnas_despues]

        # Contar las "X" en todo el DataFrame filtrado
        total_X = (df_filtrado == "X").sum().sum()

        # Contar la cantidad de "X" en cada columna
        conteo_x_por_columna = (df_filtrado == "X").sum()

        # Crear un DataFrame con los resultados
        df_conteo_X = pd.DataFrame({
            "Columna": conteo_x_por_columna.index,
            "Conteo_X": conteo_x_por_columna.values
        })

        # ==================================================
        # CUANDO SOLO SE REQUIERE CONTAR X DE LA PLANILLA
        # ==================================================
        # Crear un DataFrame con los conteos por columna
        # df_conteo_X = pd.DataFrame(list(conteo_por_columna.items()), columns=["Columna", "Conteo_X"])
        ...
        # Filtrar el DataFrame df_novedades según los valores de var_sede, var_jornada y var_grado
        # y el tipo de novedad "Cambio de complemento parcial"
        df_novedades_parcial = df_novedades[
            (df_novedades["SEDE"] == var_sede) &
            (df_novedades["JORNADA"] == var_jornada) &
            (df_novedades["GRADO_COD"] == var_grado) &
            (df_novedades["TIPO_NOVEDAD"] == "Cambio de complemento parcial")
        ]

        # Paso 1: Asegurar que df_novedades_parcial es una copia independiente para evitar advertencias
        df_novedades_parcial = df_novedades_parcial.copy()

        # Definir df_resultado_parcial como un DataFrame vacío por si no entra al if
        df_resultado_parcial = pd.DataFrame(columns=["DETALLE", "Conteo_X"])

        # Verificar si el DataFrame no está vacío antes de ejecutar el procesamiento
        if not df_novedades_parcial.empty:
            # Separar la columna DETALLE en dos partes: el número y el código
            df_novedades_parcial[["VALOR", "DETALLE"]] = df_novedades_parcial["DETALLE"].str.split("-", n=1, expand=True)

            # Convertir la columna VALOR a entero
            df_novedades_parcial["VALOR"] = df_novedades_parcial["VALOR"].astype(int)

            # Agrupar por DETALLE y sumar los valores
            df_resultado_parcial = df_novedades_parcial.groupby("DETALLE")["VALOR"].sum().reset_index()

            # Renombrar columnas
            df_resultado_parcial.columns = ["DETALLE", "Conteo_X"]

        # Filtrar el DataFrame df_novedades según los valores de var_sede, var_jornada y var_grado
        # y el tipo de novedad "Cambio de complemento"
        ...
        # Paso 1: Asegurar que df_novedades_filtro es una copia independiente para evitar advertencias
        df_novedades_filtro = df_novedades_filtro.copy()

        # Paso 2: Convertir la columna FECHA a datetime
        df_novedades_filtro['FECHA'] = pd.to_datetime(df_novedades_filtro['FECHA'], dayfirst=True)

        # Paso 3: Extraer el día del mes
        df_novedades_filtro['DIA'] = df_novedades_filtro['FECHA'].dt.day

        # Paso 4: Realizar el merge con df_conteo_X
        df_resultado = df_novedades_filtro.merge(df_conteo_X, left_on='DIA', right_on='Columna', how='left')

        # Paso 5: (Opcional) Eliminar la columna 'DIA' y 'Columna' si no es necesaria
        df_resultado.drop(columns=['Columna'], inplace=True)

        # Realizar agrupacion por tipo de complemento sumando sus celdas en "X"
        df_agrupado = df_resultado.groupby('DETALLE', as_index=False)['Conteo_X'].sum()

        # =========================================================
        # Concatenar los DataFrames validando si estan vacios o no
        # =========================================================
        if not df_resultado_parcial.empty:
            df_agrupado = pd.concat([df_agrupado, df_resultado_parcial], ignore_index=True)

            # Aplicar nuevamente el agrupamiento por DETALLE y sumar Conteo_X
            df_agrupado = df_agrupado.groupby("DETALLE")["Conteo_X"].sum().reset_index()

        # Calcular el total de "X" acumuladas en la columna 'Conteo_X'
        total_conteo_X_novedad = int(df_agrupado['Conteo_X'].sum())

        # Escribir el total de "X" acumuladas en la celda correspondiente
        if total_X > 0 and valor_J17:
            if "RPS" in valor_J17:
                # Restar el total de "X" acumuladas de acuerdo al total que aplican por novedades
                valor_RPS = total_X - total_conteo_X_novedad

                # Escribir el total de "X" en la celda G38
                worksheet.write(37, 6, valor_RPS, formato_arial)  # G38

                if (df_agrupado['DETALLE'] == 'CCT').any() and (df_agrupado['Conteo_X'].notna() & (df_agrupado['Conteo_X'] > 0)).any():
                    # Aquí va el código si se cumple la condición
                    valor_conteo = df_agrupado.loc[df_agrupado['DETALLE'] == 'CCT', 'Conteo_X'].astype(int).sum()

                    # Escribir el total de "X" en la celda G40
                    worksheet.write(39, 6, valor_conteo, formato_arial)  # G40
                else:
                    worksheet.write_blank('G40', None, borde_inferior)

                if (df_agrupado['DETALLE'] == 'RI').any() and (df_agrupado['Conteo_X'].notna() & (df_agrupado['Conteo_X'] > 0)).any():
                    # Aquí va el código si se cumple la condición
                    valor_conteo = df_agrupado.loc[df_agrupado['DETALLE'] == 'RI', 'Conteo_X'].astype(int).sum()

                    # Escribir el total de "X" en la celda G42
                    worksheet.write(41, 6, valor_conteo, formato_arial)  # G42
                else:
                    worksheet.write_blank('G42', None, borde_inferior)

            elif "CCT" in valor_J17:
                # Restar el total de "X" acumuladas de acuerdo al total que aplican por novedades
                valor_CCT = total_X - total_conteo_X_novedad

                # Escribir el total de "X" en la celda G40
                worksheet.write(39, 6, valor_CCT, formato_arial)  # G40

                if (df_agrupado['DETALLE'] == 'RPS').any() and (df_agrupado['Conteo_X'].notna() & (df_agrupado['Conteo_X'] > 0)).any():
                    # Aquí va el código si se cumple la condición
                    valor_conteo = df_agrupado.loc[df_agrupado['DETALLE'] == 'RPS', 'Conteo_X'].astype(int).sum()

                    # Escribir el total de "X" en la celda G38
                    worksheet.write(37, 6, valor_conteo, formato_arial)  # G38
                else:
                    worksheet.write_blank('G38', None, borde_inferior)
                ...
                # Restar el total de "X" acumuladas de acuerdo al total que aplican por novedades
                valor_RI = total_X - total_conteo_X_novedad

                # Escribir el total de "X" en la celda G42
                worksheet.write(41, 6, valor_RI, formato_arial)  # G42
    ...
    def separar_dataframes(self):
        ...
        for (sede, jornada, grado), df_grupo in df_focalizacion.groupby(['SEDE', 'JORNADA', 'GRADO_COD']):
            ...
            # Convertir 'GRUPO' a numérico (por si está como string)
            df_grupo['GRUPO'] = pd.to_numeric(df_grupo['GRUPO'], errors='coerce')

            # Ordenar por 'GRUPO' en orden ascendente
            df_grupo = df_grupo.sort_values(by=['GRUPO'], ascending=True)
        ...
        df_encabezado = pd.read_excel(self.ruta_archivo_aplicacion_novedades, nrows=0)

        # ==================================================================================
        # df agregado para mejorar el rendimiento cuando se realizar conteo total por grado
        # ==================================================================================

        df_focalizacion_sede = pd.read_excel(
            self.ruta_archivo_aplicacion_novedades,
            usecols=["SEDE", "JORNADA", "GRADO_COD"]
        )

        # Agrupar por 'SEDE' y 'GRADO_COD', contando cuántos registros hay en cada grupo
        # df_conteo_sede = df_focalizacion_sede.groupby(["SEDE", "GRADO_COD"]).size().reset_index(name="CANTIDAD")

        df_conteo_sede = df_focalizacion_sede.groupby(["SEDE", "JORNADA", "GRADO_COD"]).size().reset_index(name="CANTIDAD")

        # ================================================
        # Codigo para pruebas de conteos totalizados
        # ================================================
        # Obtener el índice de la columna FECHA_NACIMIENTO
        if "FECHA_NACIMIENTO" in df_encabezado.columns:
            idx_fecha_nacimiento = df_encabezado.columns.get_loc("FECHA_NACIMIENTO")
            ...
            # Agrupar por SEDE y contar las 'X' en todas las columnas posteriores a FECHA_NACIMIENTO
            conteo_x_por_sede = df_focalizacion.groupby("SEDE")[columnas_despues].apply(lambda x: (x == "X").sum().sum()).reset_index()

            # Renombrar la columna de conteo como 'TOTAL'
            conteo_x_por_sede.columns = ["SEDE", "TOTAL"]

            conteo_x_por_sede.to_excel("conteo_x_por_sede.xlsx", index=False)
        ...
        for i, (nombre_df, df) in enumerate(dfs_separados.items(), start=1):
            ...
            df_focalizacion_novedades = df_focalizacion.loc[
                (df_focalizacion["SEDE"] == var_sede) & 
                (df_focalizacion["JORNADA"] == var_jornada) & 
                (df_focalizacion["GRADO_COD"] == var_grado)
            ]

            # PRUBEBA DE FILTRO
            # if var_sede == "I.E.M. TÉCNICA AGROPECUARIA POLICARPA SALAVARRIETA - SEDE PRINCIPAL":

            self.crear_plantilla_control(df, nombre_df, var_sede, var_codigo_dane_sede, var_jornada, df_encabezado, var_grado, df_conteo_sede, df_parametros, df_focalizacion_novedades)
    ...
    def aplicar_novedades(self):
        ...
        df_asistencia = pd.read_excel(self.ruta_archivo_focalizacion)
        ...
        for index, row in df_novedades.iterrows():
            ...
            if novedad == "No hubo clases":
            ...
            elif novedad == "Ausencia especifica":
                # Obtener el número del día de la fecha
                num_dia = fecha.day  
                col_dia = int(num_dia)  # Convertir a entero para evitar problemas con float64
                ...
                # Filtrar por los documentos en detalle
                documentos = [str(doc).strip() for doc in str(detalle).split("|")]  # Convertir detalle en lista de documentos
                df_filtrado_doc = df_filtrado[df_filtrado["DOC"].astype(str).isin(documentos)]

                if df_filtrado_doc.empty:
                    print(f"Advertencia: Ningún documento en {documentos} coincide en {sede}, {jornada}, {grado}.")
                else:
                    # Dejar en blanco la celda del día correspondiente para los documentos encontrados
                    df_asistencia.loc[df_filtrado_doc.index, col_dia] = ""

            elif novedad == "Asistencia por grado":
                # Obtener el número del día de la fecha
                num_dia = fecha.day
                col_dia = int(num_dia)

                # Verificar si la columna del día existe
                if col_dia not in [int(col) for col in df_asistencia.columns if str(col).isdigit()]:
                    print(f"Error: La columna '{col_dia}' no existe en el DataFrame.")
                    return

                # Obtener los grupos desde 'detalle'
                grupos_validos = [str(grupo).strip() for grupo in str(detalle).split("|")]

                # Filtrar por los grupos válidos dentro del df_filtrado
                df_grupos_validos = df_filtrado[df_filtrado["GRUPO"].astype(str).isin(grupos_validos)]

                # Obtener los índices del mismo grado/sede/jornada que NO están en los grupos válidos
                df_grupos_no_validos = df_filtrado[~df_filtrado["GRUPO"].astype(str).isin(grupos_validos)]

                # Rellenar con "X" los grupos válidos
                df_asistencia.loc[df_grupos_validos.index, col_dia] = "X"

                # Dejar en blanco los grupos no válidos
                df_asistencia.loc[df_grupos_no_validos.index, col_dia] = ""
        ...
        df_asistencia.to_excel(self.ruta_archivo_aplicacion_novedades, index=False)
    ...
    def concatenar_exceles(self):
        ...
        # Lista para almacenar los grupos filtrados
        # listas_filtradas = []

        # for lista in listas_archivos:
        #     if any("CONCENTRACION URBANA SAN MARCOS" in str(fila) for fila in lista):
        #         print("Se encontró 'CONCENTRACION URBANA SAN MARCOS' en:", lista)
        #         listas_filtradas.append(lista)  # Agregar la lista si cumple la condición
        #     else:
        #         print("No se encontró en:", lista)
        #         # Aquí puedes definir otra acción si la lista no cumple la condición
        
        # listas_archivos = listas_filtradas  # Reemplazar la lista original por la filtrada
    ...
    def extraer_numeros(self, nombre):
        match = re.search(r'([A-ZÁÉÍÓÚÑa-zñ\s]+)_(-?\d+)_parte_(-?\d+)', nombre)

        if match:
            jornada = match.group(1).strip()
            num = int(match.group(2))
            parte = int(match.group(3))

            # Orden de prioridad para la jornada
            orden_jornadas = {"MAÑANA": 1, "TARDE": 2, "ÚNICA": 3, "MODELOS FLEXIBLES": 4}
            prioridad_jornada = orden_jornadas.get(jornada, float('inf'))

            return (num, prioridad_jornada, parte)

        return (float('inf'), float('inf'), float('inf'))

    def convertir_un_excel_a_pdf(self, archivo):
        """ Convierte un solo archivo de Excel a PDF """
        pythoncom.CoInitialize()  # Inicializar COM en este hilo
        ruta_excel = os.path.join(self.ruta_resultado_combinado, archivo)
        ruta_pdf = os.path.join(self.ruta_resultado_pdf, archivo.replace(".xlsx", ".pdf"))

        try:
            # Iniciar Excel en el hilo
            ...
            # Configurar página para cada hoja del archivo
            for ws in wb.Sheets:
            ...
            print("✅ Archivo convertido a PDF:", archivo)

        except Exception as e:
            print(f"Error al convertir {archivo}: {e}")

        finally:
            # Asegurar que Excel se cierre en este hilo
            excel.Quit()
            pythoncom.CoUninitialize()  # Liberar COM en este hilo

    def convertir_pdf(self):
        """ Convierte múltiples archivos de Excel a PDF en paralelo """
        try:
            # Registrar el tiempo de inicio
            inicio = time.time()

            # Limpiar la carpeta 'Resultado pdf' antes de generar los archivos
            self.limpiar_carpeta_resultado("Resultado pdf")
            
            print("\nConvirtiendo archivos de Excel a PDF...\n")

            if not os.path.exists(self.ruta_resultado_pdf):
                os.makedirs(self.ruta_resultado_pdf)

            archivos_excel = [f for f in os.listdir(self.ruta_resultado_combinado) if f.endswith(".xlsx")]

            # Iterar sobre cada archivo y llamar a la función convertir_un_excel_a_pdf
            for archivo in archivos_excel:
                self.convertir_un_excel_a_pdf(archivo)

            # Usar ThreadPoolExecutor para manejar múltiples hilos
            # with concurrent.futures.ThreadPoolExecutor() as executor:
            #     executor.map(self.convertir_un_excel_a_pdf, archivos_excel)
    ...
    def main(self):
        try:
            ...
            # Limpiar la carpeta 'Resultado certificaciones' antes de generar los archivos
            self.limpiar_carpeta_resultado("Resultado certificaciones")
