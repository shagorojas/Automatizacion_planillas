...
def crear_plantilla_control(df_plantilla, nombre_df, var_sede, var_codigo_dane_sede, var_jornada, df_encabezado, var_grado):
    ...
    worksheet.insert_image('A1', logo_operador, {'x_scale': 0.8, 'y_scale': 0.8})
    worksheet.insert_image('D1', logo_secretaria, {'x_scale': 0.8, 'y_scale': 0.8})
    ...
    # # Definir la columna donde se colocará la suma de "X" (AH = columna 33 en Excel)
    # columna_suma_X = 33  
    ...
    #         # Contar cuántas veces aparece "X" en las columnas especificadas para esta fila
    #         conteo_X = sum(df_plantilla[col].iloc[idx] == "X" for col in columnas_despues if col in df_plantilla.columns)
    ...
    #     if valor_J17 == "RPS":
    #         # Escribir el total de "X" en la celda G38
    #         worksheet.write(37, 6, total_X, formato_arial)  # G38
    ...
    #     elif valor_J17 == "CCT":
    #         # Escribir el total de "X" en la celda G40
    #         worksheet.write(39, 6, total_X, formato_arial)  # G40
    ...
    #     elif valor_J17 == "RI":
    #         # Escribir el total de "X" en la celda G42
    #         worksheet.write(41, 6, total_X, formato_arial)  # G42
...
def separar_dataframes():
    ...
    for i, (nombre_df, df) in enumerate(dfs_separados.items(), start=1):
        ...
        crear_plantilla_control(df, nombre_df, var_sede, var_codigo_dane_sede, var_jornada, df_encabezado, var_grado)

def limpiar_carpeta_resultado(carpeta_resultado):
...
def aplicar_novedades():
    ...
    for index, row in df_novedades.iterrows():
        ...
        # detalle = int(row["DETALLE"]) if pd.notna(row["DETALLE"]) else None
...
def main():
    ...
    limpiar_carpeta_resultado("Resultado")
...
def concatenar_exceles():

    # Limpiar la carpeta 'Resultado' antes de generar los archivos
    limpiar_carpeta_resultado("Resultado esperado")

    # Carpeta donde están los archivos
    carpeta = "Resultado"
    carpeta_salida = "Resultado esperado"
    ...
    for archivo in archivos_en_carpeta:
        # nombre_base = "_".join(archivo.split("_")[:-1])  # Quita la parte final "_parte_X"
