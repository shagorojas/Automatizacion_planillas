...
# Importamos clase para generar certificaciones
# from src.certificador import GeneradorCertificaciones
from certificador import GeneradorCertificaciones
...
class GeneradorPlantillas:
    ...
    def separar_dataframes(self):
        ...
        for i, (nombre_df, df) in enumerate(dfs_separados.items(), start=1):
            ...
            # PRUBEBA DE FILTRO
            # if var_sede == "CENTRO EDUCATIVO LA CONCEPCION":
    ...
    def extraer_numeros(self, nombre):
        match = re.search(r'([A-ZÁÉÍÓÚÑa-zñ]+)_(-?\d+)_parte_(-?\d+)', nombre)
        ...
        if match:
            ...
            orden_jornadas = {"MAÑANA": 1, "TARDE": 2, "ÚNICA": 3}
            prioridad_jornada = orden_jornadas.get(jornada, float('inf')) 

            # Orden del número principal: negativos en descendente, positivos en ascendente
            num_ordenado = (-num if num < 0 else num)  

            # Orden de la parte: negativos en ascendente, positivos en ascendente
            parte_ordenado = parte  

            return (num_ordenado, prioridad_jornada, parte_ordenado)
